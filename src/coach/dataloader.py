...
from src.dataloader import get_domain2labels, Vocab, get_vocab, PAD_INDEX
import logging
...
def read_ner(datapath, tgt_dm, src_dm, vocab):
...
def get_dataloader_for_coach(params):
    ...
    conll_inputs_train, conll_labels_train, conll_bin_labels_train = read_ner("ner_data/conll2003/train.txt", params.tgt_dm, params.src_dm, vocab)
    inputs_train, labels_train, bin_labels_train = read_ner("ner_data/%s/train.txt" % params.tgt_dm, params.tgt_dm, params.src_dm, vocab)
    ...
    inputs_dev, labels_dev, bin_labels_dev = read_ner("ner_data/%s/dev.txt" % params.tgt_dm, params.tgt_dm,  params.src_dm, vocab)

    logger.info("Load test set data ...")
    inputs_test, labels_test, bin_labels_test = read_ner("ner_data/%s/test.txt" % params.tgt_dm, params.tgt_dm,  params.src_dm, vocab)
