...
class BertTagger(nn.Module):
    def __init__(self, params):
        ...
        if self.n_layer == 1:
            ...
            self.dropout2 = torch.nn.Dropout(params.dropout)
            self.linear2 = nn.Linear(self.hidden_dim, self.num_tag)
        else:
            raise ValueError('WRONG hyperparameter for n_layers')
    ...
    def forward(self, X):
        ...
        if self.n_layer == 1:
            dropout_outp= self.dropout(outputs)
            prediction = self.linear(dropout_outp)
        elif self.n_layer == 2:
            dropout_outp = self.dropout(outputs)
            pred_layer1 = self.linear(dropout_outp)
            dropout_outp2 = self.dropout2(pred_layer1)
            prediction = self.linear2(dropout_outp2)
        else:
            raise ValueError('WRONG hyperparameter for n_layers')
